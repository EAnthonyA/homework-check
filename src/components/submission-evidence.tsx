...
export function SubmissionEvidence({ submission }: { submission: SubmissionView }) {
  const [missingPhotos, setMissingPhotos] = useState(new Set<string>());
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3">
        {submission.imagePaths.filter((src) => !missingPhotos.has(src)).map((src, index) => (
          <a key={src} href={src} target="_blank" rel="noopener noreferrer"
            ...
            <img src={src} alt={`${index + 1} namų darbų nuotrauka`} loading="lazy"
              onError={() => setMissingPhotos((current) => new Set(current).add(src))}
      ...
      {missingPhotos.size > 0 && (
        <p className="mt-2 text-xs text-ink-soft">Nuotraukos saugomos 3 dienas ir jau pašalintos.</p>
      )}
